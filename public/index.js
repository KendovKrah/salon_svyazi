...
class Tasklist {
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое название услуги', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Услуга '${taskName}' будет удалена. Продолжить?`)) return;
  ...
  render() {
    ...
    header.innerHTML = 'Стоимость:<br>' + this.tlName;
    tasklist.appendChild(header);

    const time = document.createElement('header');
    time.classList.add('tm-tasklist-header');
    time.innerHTML = 'Название тарифа:<br>' + this.tlTime;
